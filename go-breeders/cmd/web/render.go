...
func (app *application) render(w http.ResponseWriter, t string, td *templateData) {
	...
	var tmpl *template.Template //from the stdlib of Golang

	fmt.Println("DEBUG> entering render function <DEBUG")

	//IF were using the template cache try to get the template from our map, stored in the receiver.
	if app.config.useCache {

		fmt.Println("DEBUG>Using the Cache<DEBUG")
		...
		fmt.Println("DEBUG>Not using the Cache!!<DEBUG")
	}
